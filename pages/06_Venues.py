...
def get_venue_stats(venue_original_name):
    ...
    WHERE Venue = ?
    """
    df = run_query(query_match, params=(venue_original_name,))
    ...
    if not df.empty:
        ...
        total_valid = bat1_wins + bat2_wins
        stats['bat1_win_pct'] = (bat1_wins / total_valid * 100) if total_valid > 0 else 0
        stats['bat2_win_pct'] = (bat2_wins / total_valid * 100) if total_valid > 0 else 0
        stats['highest_chase'] = highest_chase if highest_chase > 0 else "N/A"
        stats['lowest_defend'] = lowest_defend if lowest_defend != 9999 else "N/A"
    else:
        return None

    q_bat = f"SELECT p.Player_Name, p.Runs_Scored FROM player_stats p JOIN innings_summary i ON p.Match_ID = i.Match_ID WHERE i.Venue = ? ORDER BY p.Runs_Scored DESC LIMIT 1"
    best_bat = run_query(q_bat, params=(venue_original_name,))
    stats['best_bat'] = f"{best_bat.iloc[0]['Player_Name']} ({best_bat.iloc[0]['Runs_Scored']})" if not best_bat.empty else "N/A"

    q_bowl = f"SELECT p.Player_Name, p.Wickets_Taken, p.Runs_Conceded FROM player_stats p JOIN innings_summary i ON p.Match_ID = i.Match_ID WHERE i.Venue = ? ORDER BY p.Wickets_Taken DESC, p.Runs_Conceded ASC LIMIT 1"
    best_bowl = run_query(q_bowl, params=(venue_original_name,))
    stats['best_bowl'] = f"{best_bowl.iloc[0]['Player_Name']} ({best_bowl.iloc[0]['Wickets_Taken']}/{best_bowl.iloc[0]['Runs_Conceded']})" if not best_bowl.empty else "N/A"
        
    q_teams = f"SELECT DISTINCT Team_Name FROM innings_summary WHERE Venue = ? ORDER BY Team_Name"
    teams_df = run_query(q_teams, params=(venue_original_name,))
    stats['teams_hosted'] = teams_df['Team_Name'].tolist() if not teams_df.empty else []
...
countries_df = run_query("SELECT DISTINCT Country FROM innings_summary WHERE Country IS NOT NULL ORDER BY Country")
available_countries = countries_df['Country'].tolist() if not countries_df.empty else ["India"]

# Sidebar Filters
selected_country = st.sidebar.selectbox("Select Host Country", available_countries)

q_venues = f"SELECT DISTINCT Venue FROM innings_summary WHERE Country = '{selected_country}' ORDER BY Venue"
venues_df = run_query(q_venues)
venue_list = ["All Venues"] + venues_df['Venue'].tolist()

selected_venue_search = st.sidebar.selectbox("Direct Venue Search", venue_list)
...
# Determine Map View
if selected_venue_search != "All Venues":
    clean_v = str(selected_venue_search).strip().lower()
    std_name = VENUE_MAP.get(clean_v, selected_venue_search)
    start_coords = VENUE_COORDS.get(std_name, COUNTRY_CENTERS.get(selected_country, [20, 0]))
    zoom_level = 12
else:
    start_coords = COUNTRY_CENTERS.get(selected_country, [20, 0])
    zoom_level = 5
...
st.subheader(f"📍 Venues in {selected_country}")
m = folium.Map(location=start_coords, zoom_start=zoom_level, tiles="CartoDB positron")
...
map_output = st_folium(m, width="100%", height=500, key="venue_map")

# --- 6. DISPLAY STATS ---
# Identify target venue from either sidebar or map click
target_db_name = None

if selected_venue_search != "All Venues":
    target_db_name = selected_venue_search
    display_name = VENUE_MAP.get(str(selected_venue_search).lower(), selected_venue_search)
elif map_output["last_object_clicked_tooltip"]:
    clicked_std_name = map_output["last_object_clicked_tooltip"]
    target_db_name = next((x['original_name'] for x in map_data if x['name'] == clicked_std_name), clicked_std_name)
    display_name = clicked_std_name

if target_db_name:
    st.divider()
    st.markdown(f"## 🏟️ Analytics: {display_name}")
    stats = get_venue_stats(target_db_name)
    ...
    st.info("👆 Click on a green pin or use the sidebar search to reveal venue intelligence.")