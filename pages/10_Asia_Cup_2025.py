...
# --- CONFIGURATION ---
st.set_page_config(page_title="Asia Cup 2025 Headquarters", page_icon="🏆", layout="wide")
MAX_OVERS = 10 
TARGET_TOUR_PATTERN = "007"  # Asia Cup 2025

# --- ALL TEAMS (Single Group) ---
ALL_TEAMS = [ "Pakistan", "India", "Bangladesh",
              "Sri Lanka", "Afghanistan"]

FULL_NAMES = {
    "SL": "Sri Lanka","AFG": "Afghanistan", "BAN": "Bangladesh",
...
# --- 2. FACTS ENGINE ---
def get_tournament_facts(TARGET_TOUR_PATTERN):
...
@st.cache_data
def get_mvp_data(TARGET_TOUR_PATTERN):
...
@st.cache_data
def get_records_data(TARGET_TOUR_PATTERN):
    conn = sqlite3.connect(DB_FILE)
...
@st.cache_data
def generate_race_data(TARGET_TOUR_PATTERN):
    ...
    try:
        ...
        WHERE Match_ID LIKE '{TARGET_TOUR_PATTERN}%' 
        AND CAST(SUBSTR(Match_ID, INSTR(Match_ID, '2025-') + 5, 2) AS INTEGER) <= 10
        ORDER BY Match_ID
        """
...
@st.cache_data
def get_final_data(TARGET_TOUR_PATTERN):
    ...
    try:
        ...
        FROM innings_summary 
        WHERE Match_ID LIKE '{TARGET_TOUR_PATTERN}%' 
        AND CAST(SUBSTR(Match_ID, INSTR(Match_ID, '2025-') + 5, 2) AS INTEGER) = 11
...
def app():
    ...
    # --- TOURNAMENT FACTS ---
    facts = get_tournament_facts(TARGET_TOUR_PATTERN)
    ...
    with tabs[0]:
        ...
        data_package = get_final_data(TARGET_TOUR_PATTERN)
    ...
    with tabs[1]:
        ...
        df_race = generate_race_data(TARGET_TOUR_PATTERN)
    ...
    # TAB 2: MVP
    with tabs[2]:
        df_mvp = get_mvp_data(TARGET_TOUR_PATTERN)
    ...
    # TAB 3: RECORDS
    with tabs[3]:
        df_bat, df_bowl, df_matches = get_records_data(TARGET_TOUR_PATTERN)
        c1, c2 = st.columns(2)
